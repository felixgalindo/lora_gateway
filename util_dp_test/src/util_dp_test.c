...
void app_log_rxpkt(struct lgw_pkt_rx_s *rxpkt)
{
    ...
    if(rxpkt->modulation == MOD_LORA){
        log_puts("LORA, %08X(%u), %d, %d, SF%dBW%d, 4/%d, %.1f, %.1f, %.1f, %.1f",
                        rxpkt->count_us,
                        rxpkt->count_us,
                        rxpkt->if_chain,
                        rxpkt->freq_hz,
...
extern uint8_t sx125x_read(uint8_t channel, uint8_t addr);
extern void read_sx125x(void);
...
int main(int argc, char **argv)
{
    ...
    if (lgw_rxrf_setconf(1, rfconf) != LGW_HAL_SUCCESS) {
        printf("WARNING: invalid configuration for radio 1\n");
    }
    ...
    ifconf.rf_chain = 1;
    ifconf.freq_hz = -300000;
    lgw_rxif_setconf(4, ifconf);

    ifconf.rf_chain = 1;
    ifconf.freq_hz = -100000;
    lgw_rxif_setconf(5, ifconf);

    ifconf.rf_chain = 1;
    ifconf.freq_hz = 100000;
    lgw_rxif_setconf(6, ifconf);

    ifconf.rf_chain = 1;
    ifconf.freq_hz = 300000;
    lgw_rxif_setconf(7, ifconf);
    ...
    txpkt.freq_hz = freq_hz+30000000;
